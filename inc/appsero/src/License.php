...
if ( ! class_exists( '\Appsero\License' ) ) {
	...
	class License {
						...
						<form method="post" novalidate="novalidate" spellcheck="false">
							...
							<div class="license-input-fields">
								...
								<button type="submit" name="submit" class="<?php echo 'deactive' === $action ? 'deactive-button' : ''; ?>">
									<?php echo 'active' === $action ? esc_html( $this->client->__trans( 'Activate License' ) ) : esc_html( $this->client->__trans( 'Deactivate License' ) ); ?>
								</button>
							</div>
						</form>
		...
		public function is_valid() {
			...
			if ( ! empty( $license['key'] ) && isset( $license['status'] ) && 'activate' === $license['status'] ) {
		...
		private function show_license_page_card_header( $license ) {
			...
			<div class="appsero-license-title">
				...
				<span><?php echo wp_kses_post( $this->client->__trans( 'Activate License' ) ); ?></span>
				...
				<form method="post" class="appsero-license-right-form" novalidate="novalidate" spellcheck="false">
					...
					<input type="hidden" name="_nonce" value="<?php echo wp_kses_post( wp_create_nonce( $this->client->name ) ); ?>">
					<button type="submit" name="submit" class="appsero-license-refresh-button">
						<span class="dashicons dashicons-update"></span>
						<?php echo wp_kses_post( $this->client->__trans( 'Refresh License' ) ); ?>
					</button>
				</form>
				<?php } ?>
